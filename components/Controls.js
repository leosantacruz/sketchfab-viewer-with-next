...
export default function Controls({ apiRef, materialList }) {
    ...
    const colorOptions = [
        { name: "Aqua", color: [32, 177, 166], classColor: "badge-accent" },
        { name: "Pink", color: [216, 39, 169], classColor: "badge-secondary" },
        { name: "Blue", color: [58, 190, 248], classColor: "badge-info" },
    ...
    const changeGroupColor = (color) => {
        const affectedMaterials = [1, 8, 10, 0]
        affectedMaterials.forEach((index) => {
    ...
    const toggleDarkMode = () => {
        ...
        let color = darkBackground ? [1, 1, 1, 1] : [17 / 255, 24 / 255, 39 / 255, 1]
    ...
    return (
        <>
            ...
            <div className="dropdown btn-block mb-3 hidden">
                ...
                {
                    !selectedMaterial && <ul tabIndex="0" className="dropdown-content menu p-2 shadow bg-base-100 rounded-box btn-block">

                        {materialList && materialList.map((material, index) => {
                            return (
                                <li key={index} onClick={() => { setSelectedMaterial(index + 1) }}><a>{material.name}</a></li>
                            )
                        })}
                    </ul>
                }
